...
def main(args):
    ...
    envs = SubprocVecEnv([make_env(DEFAULT_ENV) for i in range(N_ENVS)])
    envs = VecNormalize(envs)
    envs = VecMonitor(envs, f"./{BUILD_NAME}")
    
    # agent = PPO("CnnPolicy", envs, n_steps=N_STEPS, verbose=1)
    agent = RecurrentPPO("CnnLstmPolicy", envs, n_steps=N_STEPS, verbose=1)
